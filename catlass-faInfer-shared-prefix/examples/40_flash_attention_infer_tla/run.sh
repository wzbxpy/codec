...
function gen_data() {
    python3 examples/40_flash_attention_infer_tla/gen_prefix_suffix_data.py $batch $qSeqlen $kvSeqlen $numHeads $kvHeads $headSize $isVariedLen $maskType "$dtype" $cacheMode $layout_dtype $num_blocks $inner_prec $lse_flag
