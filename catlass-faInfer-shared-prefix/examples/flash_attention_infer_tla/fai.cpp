...
struct Options {
    ...
    static constexpr auto MIN_ARGS = 8;
...
static void Run(const Options &options) {
    ...
    int32_t accCheck = options.accCheck;
    ...
    int32_t numBlocksPre = (maxKvSeqlenPre + blockSize - 1) / blockSize;
    ...
    // );
    uint64_t blockTableSizePre = static_cast<uint64_t>(
        (maxKvSeqlenPre + blockSize - 1) / blockSize * sizeof(int32_t)
    );
