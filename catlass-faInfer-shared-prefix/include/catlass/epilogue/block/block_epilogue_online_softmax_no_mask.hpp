...
 */
 ...
 public:
     ...
     )
     {
        ...
        uint32_t gOutputHeadOffset = 0;
        if(qNBlockSize == 1) {
            gOutputHeadOffset = subblockIdx * rowSplitSubBlock;
        } else {
            gOutputHeadOffset = subBlockIdx * qNSplitSubBlock * numTokens;
        }

        AscendC::DataCopyExtParams copyOutParam = {0, 0, 0, 0, 0};
        copyOutParam.blockCount = (qNBlockSize == 1) ? 1 : qNThisSubBlock;
        copyOutParam.blockLen = (qNBlockSize == 1) ? rowActualThisSubBlock : qSBlockSize;
        copyOutParam.blockLen *= sizeof(float);
        copyOutParam.srcStride = 0;
        copyOutParam.dstStride = numTokens * sizeof(float) - copyOutParam.blockLen;
        AscendC::DataCopyPad(gOutput[gOutputHeadOffset], glUbTensor, copyOutParam);
     }
